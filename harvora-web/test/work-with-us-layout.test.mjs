...
test("booking opens a custom modal containing context and the calendar", () => {
  ...
  assert.match(ctaSource, /aria-label="Book a call with Harvora"/);
  ...
  assert.match(ctaSource, /partners@harvora\.co\.nz/);
...
test("terms contact uses the legal inbox", () => {
  ...
  assert.match(contactSection, /mailto:legal@harvora\.co\.nz/);
  assert.match(contactSection, />legal@harvora\.co\.nz</);
});

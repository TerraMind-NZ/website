...
Addendum to Harvora AI Layer PRD v2.0  
...
[1.3 Relationship to Harvora AI Layer PRD v2.0	5](#1.3-relationship-to-harvora-ai-layer-prd-v2.0)
...
[11\. Software Integration with Harvora Platform	32](#11.-software-integration-with-harvora-platform)
...
[13.5 Phase 5 — Integration Testing with Harvora Platform	37](#13.5-phase-5-—-integration-testing-with-harvora-platform)
...
[17.4 Phase 3 — Harvora Integration and API (2 weeks, parallel with Phase 2\)	44](#17.4-phase-3-—-harvora-integration-and-api-\(2-weeks,-parallel-with-phase-2\))
...
This document is the complete technical blueprint for the Harvora Soil Scouter — a field-deployable, battery-powered, wireless soil monitoring device that provides real-time soil data directly to the Harvora platform. It is a formal addendum to the Harvora AI Layer PRD v2.0 (June 2026\) and must be read alongside it.

Where the AI Layer PRD specifies the intelligence that acts on soil and environmental data, this document specifies the hardware, firmware, integration, and testing required to collect that data at the block level and stream it reliably into the Harvora data pipeline.
...
* Data schema: raw payload format, mapping to Harvora database tables
...
## **1.3 Relationship to Harvora AI Layer PRD v2.0** {#1.3-relationship-to-harvora-ai-layer-prd-v2.0}
...
This document uses the same conventions as Harvora AI Layer PRD v2.0. References to the AI Layer PRD use the format §x.x (AI PRD). References to the core PRD (Harvora PRD v1.0, May 2026\) use §x.x (Core PRD). All prices are in NZD unless otherwise specified. All times are NZST unless otherwise specified.
...
The Soil Scouter is a self-contained, weatherproof, solar-assisted sensor node that measures seven soil parameters simultaneously at the point of care in a kiwifruit orchard block. It transmits readings to the Harvora platform over WiFi (primary) or LoRa (fallback) on a configurable schedule. It requires no per-deployment subscription beyond the Harvora platform itself.
...
**Note:** All NPK and pH readings surfaced in the Harvora UI must include a disclaimer: 'Indicative reading only — confirm significant changes with laboratory analysis.' This applies to all insight, report, and briefing surfaces that display these values (AI PRD §7.4 competence boundary enforcement).

## **2.4 Scope: What the Soil Scouter Is Not** {#2.4-scope:-what-the-soil-scouter-is-not}

* Not a standalone product: the Soil Scouter has no value without the Harvora platform. It is a first-party hardware peripheral.
...
The Soil Scouter system consists of three layers: the device layer (the node in the field), the edge layer (LoRa gateway or WiFi router), and the cloud layer (Harvora platform on Railway).
...
| Cloud | Harvora Data Ingestion Service (Railway) | Receives HTTPS POST from Soil Scouter (WiFi) or LoRaWAN network server webhook (LoRa path). Validates, persists to Neon Postgres, triggers block model updates. |
| Cloud | Harvora AI Generation Service (Railway) | Consumes soil readings from database via nightly scan, prediction engine, and report generation pipelines as defined in AI PRD §5, §12, §14. |
...
7. MCU attempts WiFi connection using stored credentials. If connection succeeds within 10 seconds, payload is posted via HTTPS to Harvora data ingestion endpoint.

8. If WiFi fails (no credentials, no AP, or timeout), MCU transmits payload via LoRa using LoRaWAN OTAA on AU915 band. TTN or self-hosted gateway forwards to Harvora via webhook.
...
| HTTP Client | ESP32 Arduino HTTPClient over WiFi | HTTPS POST of JSON payload to Harvora ingestion endpoint |
...
* TLS 1.2 HTTPS with server certificate fingerprint pinned to Harvora ingestion endpoint certificate. Certificate fingerprint stored in SPIFFS config; updated via OTA firmware update when certificate rotates.
...
* On successful WiFi connection, the firmware checks a Harvora-hosted manifest file at a known URL. Manifest contains: latest\_version, firmware\_binary\_url, binary\_sha256.
...
* ESP32-S3 broadcasts a WiFi access point: SSID \= 'SoilScouter-XXXXXX' (last 6 chars of MAC address), password \= 'harvora' (printed on device label).

* Captive portal served on 192.168.4.1 — a single-page HTML form (no external CSS/JS dependencies — all inline) allowing configuration of: WiFi SSID \+ password (up to 3), Harvora device registration token, sample interval (minutes), LoRaWAN keys (if applicable).
...
Before first use, each Soil Scouter must be registered to an orchard block in Harvora. Registration assigns:

* device\_id: UUID generated by Harvora at registration time

* block\_id: the Harvora block this device monitors

* orchard\_id: inherited from block

* device\_token: a secret shared credential used to authenticate HTTPS payloads

Registration is performed via the Harvora Settings page (new 'Soil Scouters' section). The grower scans a QR code on the device label (encodes device serial number) and the platform issues device credentials. These credentials are stored to the device via the AP configuration mode portal.
...
The Harvora LoRaWAN integration server registers a webhook with The Things Network (TTN) or self-hosted ChirpStack to receive decoded uplinks and write them to the soil\_readings table (§10.2).

# **10\. Data Schema & API Integration** {#10.-data-schema-&-api-integration}

## **10.1 New Database Tables** {#10.1-new-database-tables}

Two new tables are added to the Harvora Neon Postgres instance alongside the tables defined in AI PRD §2.5:
...
New endpoints added to the Harvora application API service (Railway), alongside endpoints defined in AI PRD §17.2:
...
# **11\. Software Integration with Harvora Platform** {#11.-software-integration-with-harvora-platform}

## **11.1 Data Ingestion Service Modifications (Railway)** {#11.1-data-ingestion-service-modifications-(railway)}

The existing Harvora data ingestion service (Railway, Node.js) requires the following additions:
...
The Harvora frontend (Core PRD §7) gains the following additions for Soil Scouter data:
...
Two new alert types are added to the Harvora alert system (Core PRD §11.2):
...
| E-09 | WiFi connect and HTTPS POST | Device connects to test AP; HTTPS POST returns 201; row written to test database | Test WiFi AP, Harvora staging endpoint |
...
Deployed at pilot orchard with Harvora platform in operation. Target: 2 nodes on different blocks, 14-day validation run.

* Reading continuity: \>95% of expected readings received by Harvora over 14 days (at 15-minute intervals: expected \~1,344 readings per device; accept \>1,277 received).
...
## **13.5 Phase 5 — Integration Testing with Harvora Platform** {#13.5-phase-5-—-integration-testing-with-harvora-platform}

End-to-end integration test against Harvora staging environment:
...
| End-to-end latency (reading to Harvora) | \<5 minutes from measurement to database row written | Integration Testing |
...
57. Clone firmware repository: git clone https://github.com/harvora-io/soil-scouter-firmware
...
78. Connect phone or laptop to the AP (password: 'harvora').

79. Navigate to http://192.168.4.1 in browser.

80. Enter: WiFi SSID \+ password, Harvora registration token (obtained from Settings \> Soil Scouters in the app), sample interval (default: 15 minutes), LoRaWAN keys if applicable.

81. Submit form. Device reboots into normal operation.

82. Verify in Harvora app \> Settings \> Soil Scouters: device appears as 'active', last seen within past 5 minutes.
...
90. Confirm reading visible in Harvora app \> Block detail \> Soil panel within 20 minutes of deployment.
...
| :---- | :---- | :---- | :---- |
| NPK and pH FDR measurement accuracy is insufficient for Harvora's agronomic recommendations | Product / Data Quality | High | Framed as indicative in all UI surfaces (§2.3). Field validation against Hill Labs analysis in §13.4 will quantify accuracy gap. If gap is unacceptable, NPK/pH channels are excluded from AI feature inputs and the sensor is respecified as a 4-parameter device (moisture, temperature, EC \+ ambient). |
...
## **17.4 Phase 3 — Harvora Integration and API (2 weeks, parallel with Phase 2\)** {#17.4-phase-3-—-harvora-integration-and-api-(2-weeks,-parallel-with-phase-2)}

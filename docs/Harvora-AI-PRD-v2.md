...
Every feature in this document exists because of a Harvora prediction. The test: could this feature exist without the prediction layer? If yes, it does not belong. This filter is applied explicitly in §18.4 (Scope Risk) and was the basis for excluding insurance automation, equipment management, and general operational scheduling from this document.
...
The Reports Tab is a dedicated navigation item in the Harvora app containing four AI-generated report types: weekly, pre-season, season debrief, and agronomist. All reports are generated from data already present in the platform. No additional grower input is required at generation time beyond scheduling.
...
* Prediction accuracy — full season calibration breakdown; honest framing including where Harvora was wrong
...
* Anomalies and flagged issues — what Harvora identified as unusual with supporting data
...
The AI Season Planner is a dedicated tab in the Harvora application presenting a prediction-driven schedule of actions for the grower's orchard. Every AI-generated event exists because of a Harvora prediction. The calendar is also a logging surface — every completed event becomes a management\_actions entry.
...
* Harvora made an explicit recommendation for a specific action on a specific block
...
A simplified read-only calendar view is available to staff. The orchard Owner generates a staff access link from Settings. Staff open the link in a browser without a Harvora account. The view shows: today's scheduled actions per block in plain language, block name, action type, any timing notes from the event detail. No prediction data, finance information, or decision context is shown. The staff view auto-expires 24 hours after generation and must be refreshed by the Owner.
...
Context is scoped to what is relevant for the specific query. An Ask Harvora question about frost receives frost prediction history and frost-related management actions. It does not receive the full Psa history unless relevant. Context scoping reduces token usage and improves response quality.

## **7.3 Ask Harvora Enhancement**

The existing Ask Harvora free-text interface per block (§7.2.3 of core PRD) is unchanged in its UI. The underlying capability is substantially enhanced: the persona now receives full block history rather than the limited context available in the original specification. Response quality improves automatically with no UI change required.
...
Refusal response format: 'Harvora doesn’t have enough data to give you a reliable answer here. We recommend speaking with your agronomist / adviser.' Refusals are logged in the reports table for review during the pilot.

## **7.5 Identity**

The persona has no name, no avatar, no personality beyond Harvora's existing tone of voice (§7.6.6 of core PRD). All outputs are attributed to Harvora. Growers interact with the product, not an AI character.
...
Trust in Harvora's recommendations is built through demonstrated accuracy over time. Track record and wins surfaces show growers how accurate Harvora has been on their specific blocks and surface instances where following a recommendation led to a measurable positive outcome. Losses — where Harvora was wrong — are not surfaced here. Aggregate accuracy including misses is reported on the public calibration page.
...
* 'When Harvora predicted frost probability above X% on this block, frost occurred Y% of the time (N predictions)'
...
* Harvora recommended a protective action on a specific block
...
Every season a grower uses Harvora, predictions for their specific blocks become more accurate. The engine builds block-level models from accumulated seasonal observations. It is a background process with no direct user interface.
...
The compounding model is the most significant structural switching cost in the Harvora product. A grower who leaves after three seasons leaves behind three seasons of block-specific calibration that no competitor can access or replicate without observing the same block through the same seasons.

# **13\. Daily Briefing Tab**

## **13.1 Purpose**

The Daily Briefing Tab is the grower's first stop when they open Harvora each morning. It consolidates everything that needs attention today in a single, scannable surface. Pull only — no notification is sent; growers open it as part of their morning routine.
...
* Harvora's recommendation — with decision confidence tier
...
The grower copies this text and distributes through their own channel. Harvora provides the content; grower controls distribution. No outbound messaging from the platform.
...
Shows growers what their cumulative returns would have looked like if they had always followed Harvora's recommendations. The figure is specific to their blocks, their seasons, their decisions — it cannot be made generically and cannot be replicated by a competitor.

## **15.2 Reconstruction Logic**

For each season where Harvora has prediction data for the orchard:
...
This document contains features that sit at the boundary of Harvora's core positioning as a probabilistic forecasting and decision-support platform. The test: does the feature exist because of a Harvora prediction?
...
* AI Agronomist Persona — context assembly, Ask Harvora enhancement, competence boundary enforcement
...
| **AI identity** | No named persona; no avatar; operates as Harvora throughout |
...
Harvora AI Layer PRD v2.0  •  Confidential
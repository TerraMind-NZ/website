...
**Goal:** Collapse four overlapping project folders into one canonical Next.js app (`harvora-web/`) and delete all stale/duplicate artefacts so the repo has a single clear source of truth.

**Architecture:** The repo currently has four versions of the same site living side-by-side: a root-level static HTML file, a duplicate of that HTML in `harvora-site/`, a stale Next.js copy in `harvora-next/`, and the live/canonical Next.js app in `harvora-web/`. Only `harvora-web/` is Vercel-linked and receives commits; everything else is dead weight. The plan deletes the three dead folders and root-level cruft, leaving `harvora-web/` as the sole project inside the repo.
...
| `harvora-site/` | 100% byte-for-byte duplicate of `/index.html` + own `vercel.json` | Dead — exact duplicate |
| `harvora-next/` | Next.js port — identical structure to `harvora-web/` but 4 files are older (`Hero.tsx`, `PlatformSection.tsx`, `VisionSection.tsx`, `globals.css`) | Dead — stale intermediate step |
| `harvora-web/` | **Canonical app.** Vercel-linked (`prj_kDqBqWQUtVzJ2v4K1hKG0KGXG48a`). All recent commits touch this. | **ACTIVE — keep everything** |
...
- Delete: `vercel.json` (root) — `harvora-web/vercel.json` is the live one
- Delete: `3.png` (root)
- Delete: `harvora-site/` (entire folder)
- Delete: `harvora-next/` (entire folder)
- Modify: `README.md` (root) — update to describe the actual repo structure
- Modify: `harvora-web/CLAUDE.md` — ensure it reflects canonical status and points agents to this folder

---

## Task 1: Verify the canonical app builds cleanly before touching anything

Before deleting anything, confirm `harvora-web/` is healthy. You need a baseline.

**Files:** `harvora-web/` (read-only verification)

- [ ] **Step 1: Run type-check**

```bash
cd harvora-web && npx tsc --noEmit
```
...
```bash
cd harvora-web && npm run dev -- --port 3001
```
...
```bash
ls harvora-site 2>/dev/null && echo "ERROR: still exists" || echo "OK: deleted"
```
...
```bash
git add -A
git commit -m "chore: remove harvora-site — exact duplicate of root index.html"
```

---

## Task 3: Delete `harvora-next/`

This folder is a stale Next.js copy identical to `harvora-web/` except 4 files are older versions. It is not linked to Vercel and has received no commits since `708c60c`.

**Files:** Delete `harvora-next/`

- [ ] **Step 1: Confirm which files differ (for the record)**

```bash
diff -rq --exclude="node_modules" --exclude=".next" --exclude="package-lock.json" harvora-next/src harvora-web/src
```

Expected: only `Hero.tsx`, `PlatformSection.tsx`, `VisionSection.tsx`, `globals.css` differ. `harvora-web/` has the newer versions of all four.
...
```bash
ls harvora-next 2>/dev/null && echo "ERROR: still exists" || echo "OK: deleted"
```
...
```bash
git add -A
git commit -m "chore: remove harvora-next — stale intermediate copy, superseded by harvora-web"
```
...
The root `index.html`, `vercel.json`, and `3.png` belong to the pre-Next.js era. The Vercel deployment now uses `harvora-web/vercel.json`.
...
```bash
git add -A
git commit -m "chore: remove root index.html, vercel.json, 3.png — HTML-era artefacts replaced by harvora-web"
```
...
Marketing website for [Harvora](https://harvora.co.nz) — agricultural intelligence for horticulture growers.
...
```bash
git add README.md
git commit -m "docs: update root README to reflect harvora-web as sole project"
```

---

## Task 6: Update `harvora-web/AGENTS.md` to reflect canonical status

`harvora-web/CLAUDE.md` imports `AGENTS.md` via `@AGENTS.md`, so canonical context belongs in `AGENTS.md` — both files stay in sync automatically without duplication.

**Files:** Modify `harvora-web/AGENTS.md`

- [ ] **Step 1: Read current CLAUDE.md and AGENTS.md**

```bash
cat harvora-web/CLAUDE.md
cat harvora-web/AGENTS.md
```

- [ ] **Step 2: Update `harvora-web/AGENTS.md`**
...
This directory (`harvora-web/`) is the **canonical source of truth** for the Harvora marketing website. It is the only active project in this repository.
...
Confirm the repo is clean and `harvora-web/` still works.
...
Expected: only `docs/`, `harvora-web/`, `README.md`, `.gitignore`, `.git` (and `.DS_Store` on macOS is fine). No `index.html`, no `vercel.json`, no `3.png`, no `harvora-site/`, no `harvora-next/`.
...
- `harvora-web/.vercel/` folder is intentionally kept — it stores the Vercel project link and must not be deleted.
- `harvora-web/node_modules/` is in `.gitignore` and is not tracked; no action needed.
